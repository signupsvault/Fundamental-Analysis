...
function showAddCompanyForm() {
  ...
  updateCompanyList();
    ...
    if (currentCompanyIndex !== -1) {
        ...
        const listItems = companyListElement.querySelectorAll('li');
  ...
  updateCompanyListHighlight();
  document.getElementById('fundamentalAnalysis').style.display = 'none'; //hide the fundametal analysis section.
    document.getElementById('fileDisplay').innerHTML = '<p>No files uploaded</p>'; //clear the display of uploaded files
}
...
function handleFileUpload(event) {
    ...
    // Reset label to default state with icon and text
    const resetLabel = () => {
        fileInputLabel.innerHTML = '';
        const uploadIcon = document.createElement('i');
        uploadIcon.classList.add('fas', 'fa-upload');
        fileInputLabel.appendChild(uploadIcon);
        fileInputLabel.appendChild(document.createTextNode(' Upload File'));
    };
    ...
    if (file.type && file.type.startsWith('image/')) {
       ...
       fileInputLabel.appendChild(fileIcon);
        fileInputLabel.appendChild(document.createTextNode(' '));
...
function displayFiles(companyName) {
  ...
  savedFiles.forEach((file, index) => {
    ...
    // Create file type icon element
    const fileIcon = document.createElement('i');
    fileIcon.classList.add('file-icon');

    // Dynamically assign icon based on file type
     if (file.type && file.type.startsWith('image/')) {
      fileIcon.classList.add('fas', 'fa-file-image'); // Image icon
    } else if (file.type && file.type === 'application/pdf') {
        fileIcon.classList.add('fas', 'fa-file-pdf'); // PDF icon
    } else if (file.type && file.type.startsWith('text/')) {
        fileIcon.classList.add('fas', 'fa-file-alt'); // Text icon
    }
    else {
    ...
    fileNameSpan.textContent = file.boxName;
    fileNameSpan.style.cursor = 'pointer';
      fileNameSpan.addEventListener('click', e => {
            e.preventDefault();
            openFilePreviewModal(file); //open in modal
        });
    // Create rename input
    const renameInput = document.createElement('input');
    renameInput.type = 'text';
    renameInput.value = file.boxName;
    ...
    renameInput.addEventListener('blur', (e) => {
      ...
      if (newName && newName !== file.boxName) {
        renameFileBox(companyName, index, newName);
      } else {
        renameInput.style.display = 'none';
    ...
    fileLink.classList.add('file-link');
      const fileLinkElement = document.createElement('span');
         fileLinkElement.textContent = file.name;
       fileLink.appendChild(fileLinkElement)
    const deleteButton = document.createElement('button');
    ...
    fileNameContainer.appendChild(fileIcon); // Append the icon first
    fileNameContainer.appendChild(fileNameSpan); // Append name
      fileNameContainer.appendChild(renameInput);
      fileNameContainer.appendChild(renameButton);
...
function renameFileBox(companyName, fileIndex, newName) {
      let savedFiles = JSON.parse(localStorage.getItem(`companyFiles-${companyName}`) || '[]');
    if (fileIndex >= 0 && fileIndex < savedFiles.length) {
          savedFiles[fileIndex].boxName = newName;
      localStorage.setItem(`companyFiles-${companyName}`, JSON.stringify(savedFiles));
      displayFiles(companyName);
    }
}
...
function openFilePreviewModal(file) {
    ...
    maximizeButton.addEventListener('click', maximizeFilePreviewModal);
         controls.appendChild(maximizeButton);
         modal.appendChild(controls);
  if(file.type && file.type.startsWith('image/')) {
          filePreviewElement = document.createElement('img');
         filePreviewElement.src = file.url;
    ...
    function closeFilePreviewModal() {
        document.getElementById('filePreviewModal').style.display = 'none';
         const controls = document.getElementById('filePreviewModal').querySelector('.controls');
            if (controls) {
               controls.remove();
            }
         document.getElementById('modalOverlay').style.display = 'none';
         document.getElementById('filePreviewModal').classList.remove('maximized');
    }
    function makeModalDraggable() {
        let modal = document.getElementById('filePreviewModal');
        modal.style.cursor = 'default'; // Remove cursor move
    }
    function maximizeFilePreviewModal() {
        const modal = document.getElementById('filePreviewModal');
       modal.classList.toggle('maximized');
    }
    function updateCompanyListHighlight() {
        const companyListElement = document.getElementById('companyListDropdown');
        const listItems = companyListElement.querySelectorAll('li');
